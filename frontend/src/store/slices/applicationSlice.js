...
export const fetchEmployerApplications=()=>async(dispatch)=>{
    ...
    try {
        const response =await axios.get("http://localhost:4000/api/application/employer/getAll",
...
export const fetchJobSeekerApplications=()=>async(dispatch)=>{
    ...
    try {
        const response =await axios.get("http://localhost:4000/api/application/jobseeker/getAll",
            {withCredentials:true                  });
...
export const postApplication=(data,jobId)=>async(dispatch)=>{
    ...
    try {
        const response =await axios.post(`http://localhost:4000/api/application/post/${jobId}`,data,{withCredentials:true,headers:{
            "Content-Type":"multipart/form-data"
        }});

        console.log(response);
        dispatch(applicationSlice.actions.successForPostApplication(response.data.application));
        dispatch(applicationSlice.actions.clearAllErrors());
    } catch (error) {
...
try {
    const response =await axios.delete(`http://localhost:4000/api/application/delete/${id}`,
        {withCredentials:true});
